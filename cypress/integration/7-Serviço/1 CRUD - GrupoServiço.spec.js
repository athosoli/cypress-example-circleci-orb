...
context("Contas a Receber", () => {
  ...
  it("Criar e Ler - Grupo de Serviços", () => {
    if (cy.get('#navigation-menu').should("be.visible")) {
      ...
      cy.wait(1000);
      cy.get(loc.servico.grupoServico).click({ force: true });
      cy.wait(5000);
    ...
    cy.get('#prefixo').type("4UTO");
    cy.get('#descricao').type("AUTOMOTIVOS");
    //SALVAR
    ...
    cy.contains("AUTOMOTIVOS").dblclick({ force: true });
  
    cy.wait(1000)
    cy.get(loc.menu.caminho2).should('contain', 'Visualizar')
    cy.get(loc.integracao.Corpo).should('contain','AUTOMOTIVOS')
    cy.get(loc.integracao.Corpo).should('contain','4UTO')
    cy.get('.pull-right > .md-button > span.ng-scope').click({ force: true })
  });
  ...
  it("Editar - Grupo de Serviços", () => {
    ...
    cy.contains("AUTOMOTIVOS").click({ force: true });
    cy.get('#btn-editar > span.ng-scope').click({ force: true })
    
    cy.get('#descricao').type("MARITIMO")
    //SALVAR
    cy.get('#btn-salvar > span.ng-scope').click({force:true})
    ...
    cy.contains("MARITIMO").dblclick({ force: true })


    cy.wait(1000)
    cy.get(loc.menu.caminho2).should('contain', 'Visualizar')
    cy.get(loc.integracao.Corpo).should('contain','MARITIMO')
  ...
  it("Exluir - Grupo de Serviços", () => {
    ...
    cy.contains("MARITIMO").click({ force: true })
